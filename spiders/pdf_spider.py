...
class PdfSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for category_link in response.css('a.preFade.fadeIn::attr(href)').getall():
            yield response.follow(category_link, callback=self.parse_category)

    def parse_category(self, response):
        # Find product links and next page link
        for product_link in response.css('a.blog-more-link.preFade.fadeIn::attr(href)').getall():
            yield response.follow(product_link, callback=self.parse_product, dont_filter=True)
    ...
    def parse_product(self, response):
        ...
        google_drive_link = response.css('p.preFade.fadeIn > a::attr(href)').get()
        ...
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            ...
            with pdfplumber.open(temp_file_path) as pdf:
                for page in pdf.pages:
                    # Extract tables
                    tables = tabula.read_pdf(temp_file_path, pages=page.page_number, multiple_tables=True) 
                    for table in tables:
